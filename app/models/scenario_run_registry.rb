...
class ScenarioRunRegistry < ActiveRecord::Base
  ...
  def self.latest_run
    return nil unless ScenarioRunRegistry.last
    ScenarioRunRegistry.last # Can we count on last to always be the one with the latest dt_stamp?
  ...
  def self.save_run(scenarios_results:)
    run = ScenarioRunRegistry.create(dt_stamp: dt_stamp_now_et)
    scenarios_results.each { |result| ScenarioRunHistory.save_result(run_id: run.id, scenario_result: result) }
  end
