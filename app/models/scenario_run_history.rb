...
class ScenarioRunHistory < ActiveRecord::Base
  ...
  self.summary_class = ScenarioRunSummary
  ...
  def self.save_result(run_id:, scenario_result:)
    ScenarioRunHistory.create(scenario_run_registry_id: run_id,
  ...
  def self.run_results(run_id:, authority_name: nil, status: nil, url: nil)
    ...
    ScenarioRunHistory.where(where).to_a
  end
  ...
  def self.run_failures(run_id:)
    return [] unless run_id
    ScenarioRunHistory.where(scenario_run_registry_id: run_id).where.not(status: :good).to_a
  end
  ...
  private
    def self.authorities_in_run(run_id:)
      ScenarioRunHistory.where(scenario_run_registry_id: run_id).pluck(:authority_name).uniq
    end

    def self.authorities_with_failures_in_run(run_id:)
      ScenarioRunHistory.where(scenario_run_registry_id: run_id).where.not(status: 'good').pluck('authority_name').uniq
    end

    def self.status_counts_in_run(run_id:)
      status = ScenarioRunHistory.group('status').where(scenario_run_registry_id: run_id).count
    ...
    def self.all_runs_per_authority
      authority_runs = ScenarioRunHistory.pluck(:authority_name, :scenario_run_registry_id).uniq
      runs_per_authority(authority_runs)
    end

    def self.failing_runs_per_authority
      failing_authority_runs = ScenarioRunHistory.where.not(status: 'good').pluck(:authority_name, :scenario_run_registry_id).uniq
