...
class SearchScenarioValidator < ScenarioValidator
  ...
  private
    ...
    def subject_position(results, subject_uri, total_run_time)
      ...
      log(status: UNKNOWN, errmsg: "Search position scenario failed; cause: subject uri (#{subject_uri}) not found in results", expected: scenario.expected_by_position, target: subject_uri, normalization_run_time: total_run_time) # TODO: need to get run times from results
      return nil
    end
end
